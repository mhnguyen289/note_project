...
function bindClick() {
  ...
  $('.js-prev').on('click', function(e) {
    ...
    fetch(`/api/notes/next`)
      .then(res => res.json() )
      .then(note => {
        const newPost = new Post(note.id, note.note_title, note.note_content)
        const noteHTML = newPost.formatPost()
  ...
  $('.js-next').on('click', function(e) {
    ...
    fetch(`/api/notes/${id + 1}`)
      .then(res => res.json() )
      .then(note => {
        const newPost = new Note(note.id, note.note_title, note.note_content)
        const noteHTML = newPost.formatPost()
        $('#app-container').append(noteHTML)
      })
      .catch(err => console.log(err))
